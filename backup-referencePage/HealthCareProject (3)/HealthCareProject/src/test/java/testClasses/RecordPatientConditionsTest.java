...
public class RecordPatientConditionsTest extends BaseClass{
	...
	public static void recordPatientCondition() throws InterruptedException, AWTException
	{
...
//		searchbox_Condition.sendKeys(Keys.RETURN);
		...
		By suggestionList = By.xpath("//ul[contains(@class, '-esm-patient-conditions__conditions-form__conditionsList___e24pL')]");
		wait.until(ExpectedConditions.visibilityOfElementLocated(suggestionList));

		// Wait for the specific condition suggestion to be visible
		By suggestionItem = By.xpath("//li[contains(@class, '-esm-patient-conditions__conditions-form__condition___4ZLvq') and contains(text(), '" + conditionName + "')]");
		WebElement suggestion = wait.until(ExpectedConditions.visibilityOfElementLocated(suggestionItem));

		// Click the suggestion using JavaScriptExecutor
		((JavascriptExecutor) driver).executeScript("arguments[0].click();", suggestion);
		
		//
//		String conditionName="Paratyphoid Fever";
//		
//		addExplicitWait(RecordPatientConditionsPage.clickONSearchCondition());
//		WebElement searchbox_Condition = driver.findElement(RecordPatientConditionsPage.clickONSearchCondition());		
//		searchbox_Condition.sendKeys(conditionName); 
...
//		  
//		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(60));
//		By suggestionItem = By.xpath("//li[contains(@class, 'conditions-formcondition')]");
//		 // By suggestionItem=By.xpath("(//li[@class='-esm-patient-conditions_conditions-formcondition__4ZLvq'])[1]")));		
//		List<WebElement> suggestions = wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(suggestionItem));
//
//		boolean matched = false;
//		for (WebElement suggestion : suggestions) {
//		    System.out.println("Suggestion found: " + suggestion.getText()); // debug
//		    if (suggestion.getText().equalsIgnoreCase(conditionName)) {
//		        suggestion.click();
//		        matched = true;
//		        break;
//		    }
...
//
//		if (!matched) {
//		    throw new RuntimeException("Condition suggestion not found for: " + conditionName);
//		}
...
//
//		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(60));
//		By suggestionItem = By.xpath("//li[@class='-esm-patient-conditions__conditions-form__condition___4ZLvq']//font[text()='Depression']");
//
//		List<WebElement> suggestions = wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(suggestionItem));
//
//		boolean matched = false;
...
//		    System.out.println("Suggestion found: " + suggestion.getText()); // debug
//		    if (suggestion.getText().equalsIgnoreCase(conditionName)) {
//		        // 👇 Use JavaScriptExecutor to click the suggestion
//		        ((JavascriptExecutor) driver).executeScript("arguments[0].click();", suggestion);
//		        matched = true;
//		        break;
//		    }
...
//		 addExplicitWait(RecordPatientConditionsPage.enterOnSetDate());
//	     driver.findElement(RecordPatientConditionsPage.enterOnSetDate()).sendKeys("13/05/2025");
...
//		saveButton.click();
		...
		RecordPatientConditionsPage.verifyConditionAdded(driver,conditionName);
